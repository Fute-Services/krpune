...
function createWindow() {
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // Offline kiosk — only loads local bundled assets over file://.
      // On some GPUs/drivers, WebGL treats file:// images as cross-origin and
      // refuses to use them as textures, so every 360° panorama (pannellum VR
      // + three.js amenities) renders pure black. Disabling webSecurity removes
